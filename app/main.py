...
async def convert_media(file:UploadFile=File(...)
                        ,target_format:str=Query("avi",description="the target format extension to convert to")): #(...) makes upload mandatory
    target_format=target_format.lower().strip().replace(".","")
    if target_format not in ALLOWED_TARGET_FORMATS:
        raise HTTPException(
            status_code=400,
            detail="unsupported format , choose target format from ,choose from: {','.join(ALLOWED_TARGET_FORMATS)}"
        )
    if not file.filename.endswith(".mp4"):
        raise HTTPException(status_code=400,detail="only .mp4 files are supported")
    input_path=os.path.join(UPLOAD_DIR,file.filename)
    base_filename=file.filename.rsplit(".",1)[0]
    output_filename=f"{base_filename}.{target_format}"#file.filename.rsplit(".",1)[0]+".avi" #splits like ["my_movie", "mp4"]
