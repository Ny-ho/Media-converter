...
@celery_app.task
def auto_cleanup_janitor_task():
    print("janitor task:starting disk cleanup audit..")
    target_directories=["data/uploads","data/outputs"]
    EXPIRATION_SECONDS=10#86400
    ...
    for directory in target_directories:
        ...
        for filename in os.listdir(directory):
            ...
            if file_age_Seconds>EXPIRATION_SECONDS:
                print(f"janitor task:deleting experied files -> {file_path}")
                os.remove(file_path)
                deleted_files_count+=1

        print(f"janitor cleaned the unnecessary files :{deleted_files_count} number of files removed")
        return{"status":"success","deleted fikes":deleted_files_count}
